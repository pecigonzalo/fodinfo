...
```console
$ helm repo add fodinfo https://pecigonzalo.github.io/fodinfo

$ helm upgrade -i my-release fodinfo/fodinfo
```

The command deploys fodinfo on the Kubernetes cluster in the default namespace.
...
The following tables lists the configurable parameters of the fodinfo chart and their default values.
...
`image.repository` | `pecigonzalo/fodinfo` | Image repository
...
`service.httpPort` | `5000` | Container HTTP port
`service.externalPort` | `5000` | ClusterIP HTTP port
`service.grpcPort` | `9999` | ClusterIP gPRC port
`service.grpcService` | `fodinfo` | gPRC service name
...
`securityContext` | `{}` | The security context to be set on the fodinfo container
...
```console
$ helm install my-release fodinfo/fodinfo \
...
```console
$ helm install my-release fodinfo/fodinfo -f values.yaml
```
